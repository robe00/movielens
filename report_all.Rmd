...
# Suppress summarise info - annoying in report and does not help
...
Machine learning helps extract useful insights from data. Effective machine learning algorithms are used to obtain these insights within a short period of time.
...
```{r initial_load1, warning=FALSE, message=FALSE, echo=FALSE}
...
```{r initial_inquiries0a, echo=FALSE}
...
```{r initial_inquiries0b, echo=FALSE}
...
```{r initial_inquiries0c, echo=FALSE}
...
```{r initial_inquiries1, echo=FALSE}
...
here grouped by title and sorted descending by no. of ratings:

```{r initial_inquiries2, echo=FALSE}
...
here grouped by title, filtered by "no_of_ratings==1" and the result counted.

```{r initial_inquiries3, echo=FALSE}
...
```{r initial_inquiries4a, echo=FALSE}
...
```{r initial_inquiries4b, echo=FALSE}
...
The mean rating distribution show slightly skewness - that can be used as influencing factor for our predicting algorithm.

The distribution of user ratings shows that many users have watched or rated less than half of all movies.

```{r initial_inquiries5b, echo=FALSE}
...
When we put the relation of user vs. movies  in a cross tab to see what user has rated what movie, the result a sparse,
but very big matrix - here only shown a small subset of 100 user (rows) and 200 movies (columns):

All nonzero values are represented by a dot. Empty spaces means no ratings were given by the user. 

```{r initial_inquiries6, echo=FALSE}
...
```{r initial_inquiries7, echo=FALSE}
...
```{r initial_inquiries8, warning=FALSE, message=FALSE, echo=FALSE}
...
When we check the frequency of movies per year we see an exponential growth of the movie business up to 2010 and then a sharp decrease,
does not mean the movie business went bancrupt but lack of data, in the dataset were no younger data.

```{r initial_inquiries9a, echo=FALSE}
...
movies_per_year %>%
  ...
  labs(caption = "exponential growth of the movie business up to 2010 - no younger data")
...
```{r initial_load2, warning=FALSE, message=FALSE, echo=TRUE}
...
```{r Loss function RSME, echo=FALSE}
...
```{r movie_effect_model1_1, echo=FALSE}
...
```{r movie_effect_model1_2, echo=FALSE}
...
compare it by RSME with reality and add the following result for RMSE to the summary table.

```{r movie_effect_model1_3, echo=FALSE}
...
```{r movie_user_effect_model2_1, echo=FALSE}
...
```{r movie_user_effect_model2_2, echo=FALSE}
...
```{r movie_user_effect_model2_3, echo=FALSE}
...
```{r movie_user_genre_effect_model3_1, echo=FALSE}
...
```{r movie_user_genre_effect_model3_2, echo=FALSE}
...
```{r movie_user_genre_time_effect_model4_1, echo=FALSE}
...
```{r movie_user_genre_time_effect_model4_2, echo=FALSE}
...
it seems it is showing just noise but no skewness. 
...
```{r movie_user_genre_time_effect_model4_3, echo=FALSE}
...
We test `lamda <- seq(from=0, to=10, by=0.25)`
...
```{r regularized_effects5_2, echo=FALSE}
...
```{r regularized_effects5_3a, echo=FALSE}
...
```{r regularized_effects5_3b, echo=FALSE}
...
```{r regularized_effects6_2, echo=FALSE}
...
```{r regularized_effects6_3a, echo=FALSE}
...
```{r regularized_effects6_3b, echo=FALSE}
...
```{r regularized_effects7_2, echo=FALSE}
...
```{r regularized_effects7_3, echo=FALSE}
...
```{r regularized_effects7_3b, echo=FALSE}
...
if(FALSE) {
  ...
  write.csv(validation,"predicted_movie_ratings.csv", na = "")
  write.table(BESTRMSE,"RMSE_score.csv"             , na = "", row.names=FALSE,col.names=FALSE)
}
```
...
So one has to think about whether it makes sense to consider i.e. genres or dates because
that improves the predictions only at the 3rd or 4th place behind the decimal point.

For future work: the distribution of the ratings per genres over time was just slightly touched, 
remarked they seem to follow the curve of the movies per year.
A further approach would be to normalize these genre curves (divide by numbers of movies in the year) 
and see whether that has an effect on quality of the predictions.

## Addendum

### Benchmark: time to generate Pdf from Rmd with the small and big dataset
